...
# ── shared utils (same as classical ML models) ──────────────────────────────
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from utils.config_utils import load_config
from utils.io_utils import ensure_dir, save_csv, save_json
from utils.metric_utils import compute_binary_metrics, sweep_thresholds_for_f1
...
def ensure_memmap(cfg: dict, chunksize: int = 50_000) -> None:
    ...
    from tabnet.prepare_memmap import (
        build_label_mapping,
