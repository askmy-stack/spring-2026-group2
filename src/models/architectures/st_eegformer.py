...
class _PatchEmbed(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, int]:
        ...
        if samples % self.patch_size != 0:
            raise ValueError(f"ST-EEGFormer expects samples divisible by patch size {self.patch_size}, got {samples}.")
...
class _TemporalEncoding(nn.Module):
    def __init__(self, max_patches: int, embed_dim: int):
        ...
        _, max_patches, width = self.pe.shape
        ...
        pe = torch.zeros(max_patches, width, dtype=torch.float32)
...
class HFSTEEGFormerPretrainedModel(nn.Module):
    ...
    def _load_pretrained_weights(self, pretrained_repo: str, filename: str):
        try:
            ...
            raise ImportError(
                "ST-EEGFormer requires `huggingface_hub` and `safetensors`. Install them with `pip install huggingface_hub safetensors`."
            ) from exc
...
__all__ = ["HFSTEEGFormerPretrainedModel"]
