...
Inherits from BaseDatasetLoader. All dataset-specific logic lives here:
    - Downloads from URL specified in config.yaml
    - Parses subject demographics from dataset metadata files
    - Parses Seizures-list-PNXX.csv for seizure start/end times

All constants (URL, sfreq, notch, paths) come from config.yaml → datasets.siena.
...
def _load_config(config_path: Optional[str] = None) -> Dict:
    """Load dataset config from config.yaml → datasets.siena."""
    ...
    dataset_cfg = full.get("datasets", {}).get("siena", {})
    if not dataset_cfg:
        raise KeyError("No 'datasets.siena' section found in config.yaml")

    # Merge with global signal config for target_sfreq
    signal_cfg = full.get("signal", {})
    dataset_cfg.setdefault("target_sfreq", signal_cfg.get("target_sfreq", 256))

    return dataset_cfg


class SienaLoader(BaseDatasetLoader):
    """Siena Scalp EEG loader — reads all parameters from config.yaml."""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            raw_cache=raw_cache or cfg.get("raw_cache", "data/raw/siena"),
            native_sfreq=cfg.get("native_sfreq", 512.0),
            target_sfreq=cfg.get("target_sfreq", 256),
            notch_freq=cfg.get("notch_freq", 50.0),
            population=cfg.get("population", "adult"),
            force=force,
        )
        self._subject_info_parsed = False
        self._all_metadata: Dict[str, Dict[str, Any]] = {}
        self._metadata_files = cfg.get("metadata_files", [
            "subject-info.csv", "SUBJECT-INFO",
            "subject-info", "participants.tsv",
        ])
    ...
    def _load_subject_info(self):
        ...
        Tries filenames from config.yaml → datasets.siena.metadata_files.
        """
        parsed = False

        for filename in self._metadata_files:
