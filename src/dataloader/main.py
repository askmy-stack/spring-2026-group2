...
def quick_verify(loader, num_samples=10):
    ...
    for i in range(min(num_samples, len(loader))):
        ...
        target_sfreq = cfg.get("signal", {}).get("target_sfreq", 256)
        win_sec = cfg.get("windowing", {}).get("window_sec", 1.0)
        expected_samples = int(round(target_sfreq * win_sec))

        # channels in cleaned FIF might not be 16; don't hard fail unless you enforce it
...
def run_full_pipeline():
    ...
    try:
        loader = UniversalEEGLoader(config_path="src/dataloader/configs/config.yaml", mode="train")
        ...
        for mode in ['train', 'val', 'test']:
            try:
                dataset = UniversalEEGLoader(config_path="configs/config.yaml", mode=mode)
