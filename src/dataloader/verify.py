...
def test_class_hierarchy():
    ...
    from src.dataloader.dataloader import BaseEEGLoader
...
def test_standard_loader():
    ...
    # Generic assertions (do NOT hardcode shape)
    assert data.ndim == 2, f"Expected (n_channels, n_samples), got {tuple(data.shape)}"
    assert data.shape[0] >= 1, f"Expected >=1 channel, got {data.shape[0]}"
    assert data.shape[1] >= 8, f"Expected >=8 samples, got {data.shape[1]}"
    assert label.item() in [0, 1], f"Label must be 0/1, got {label.item()}"

    print(f"Loaded {len(loader)} windows | sample shape={tuple(data.shape)}")
    return True
...
def test_enhanced_loader():
    ...
    data, label = loader[0]
    assert data.ndim == 2, f"Expected (n_channels, n_samples), got {tuple(data.shape)}"
    assert data.shape[0] >= 1 and data.shape[1] >= 8
    assert label.item() in [0, 1]
    print(f"Sample shape={tuple(data.shape)}")
    return True
...
def test_config_compatibility():
    ...
    configs = [
        "src/dataloader/configs/config.yaml",
        "src/dataloader/configs/config_v2.yaml",
        "config.yaml",
        "config_v2.yaml",
    ]
